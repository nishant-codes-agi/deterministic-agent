    ...
    def create_visualization(self, output_file: str = "anomalies.png") -> bool:
        ...
        try:
            ...
            plt.plot(self.data.index, self.data['Close'], 
                    label=f'{self.ticker} Close Price', color='blue', alpha=0.7)
            ...
            plt.title(f'{self.ticker} Stock Price with Anomaly Detection\n(IQR Method, Multiplier: {self.iqr_multiplier})')
            plt.xlabel('Date')
            plt.ylabel('Price ($)')
            plt.legend()
        ...
        try:
            ...
            report = {
                ...
                "statistics": {
                    ...
                    "std_price": float(self.data['Close'].std()),
            ...
            if self.anomalies is not None and len(self.anomalies) > 0:
                for date, row in self.anomalies.iterrows():
                    ...
                    report["anomalies"].append(anomaly_data)
